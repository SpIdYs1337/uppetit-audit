...
export function useSchedule() {
  ...
  // SWR не начнет запрос планов, пока не получит userId
  const { data: plans, mutate: mutatePlans, isLoading: plansLoading } = useSWR<any[]>(
  ...
  const addPlan = async (locationId: string, date: string) => {
    ...
    mutatePlans(); // Обновляем кэш
  };

  const deletePlan = async (id: string) => {
    // Оптимистичное удаление: сразу прячем карточку, не дожидаясь сервера
    mutatePlans(plans?.filter(p => p.id !== id), false);
    
    const res = await fetch(`/api/schedule?id=${id}`, { method: 'DELETE' });
    if (!res.ok) {
      mutatePlans(); // Возвращаем карточку, если сервер выдал ошибку
      throw new Error('Ошибка удаления');
    }
  };

  // --- ЛОГИКА ГРУППИРОВКИ И ФОРМАТИРОВАНИЯ ---
