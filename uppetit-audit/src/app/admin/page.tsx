  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Ошибка при сохранении. Возможно, такой логин уже существует.'); 
      }
    } catch (err) { 
      ...
      {isEditing ? (
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-8 max-w-2xl">
          ...
          <div className="mt-6 border-t border-gray-100 pt-6">
            <button onClick={handleSave} className="w-full bg-[#F25C05] text-white py-4 rounded-xl font-bold shadow-lg hover:brightness-110 active:scale-[0.98] transition-all">Сохранить</button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {users.map(user => {
            ...
            // Проверка: пароль установлен, если токена нет или он помечен как 'USED_'
            const isPasswordSet = !user.inviteToken || String(user.inviteToken).startsWith('USED_');
