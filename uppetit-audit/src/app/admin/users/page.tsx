...
export default function AdminUsersPage() {
  ...
  // Фильтрация и разделение пользователей
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto pb-20">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Администрирование</h1>
      ...
      {isEditorOpen ? (
        ...
        <>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
            ...
            {/* Вкладки */}
            <div className="flex bg-gray-100/50 p-1 rounded-2xl border border-gray-200/50 w-full lg:w-auto overflow-x-auto">
              <button 
                onClick={() => setActiveTab('EMPLOYEES')}
                className={`flex-1 lg:flex-none px-6 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap ${activeTab === 'EMPLOYEES' ? 'bg-[#F25C05] text-white shadow-md' : 'text-gray-500 hover:text-gray-900 hover:bg-white/50'}`}
              >
                Сотрудники ({activeUsers.length})
              </button>
              <button 
                onClick={() => setActiveTab('INVITES')}
                className={`flex-1 lg:flex-none px-6 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap ${activeTab === 'INVITES' ? 'bg-[#F25C05] text-white shadow-md' : 'text-gray-500 hover:text-gray-900 hover:bg-white/50'}`}
              >
            ...
            <div className="w-full lg:w-72 relative">
              <input 
                ...
                // ИЗМЕНЕНО: Добавлены text-gray-900 и placeholder-gray-400
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-900 placeholder-gray-400 outline-none focus:border-[#F25C05] transition-colors shadow-sm"
              />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {activeUsers.length === 0 ? (
                ...
                activeUsers.map(user => (
                  <div key={user.id} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-xl font-black text-gray-900 leading-tight">{user.name || 'Имя не указано'}</h3>
                    ...
                    <div className="mb-4 space-y-1">
                      {user.email && <div className="text-sm text-gray-500 font-bold truncate">📧 {user.email}</div>}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {invitedUsers.length === 0 ? (
                ...
                invitedUsers.map(user => (
                  <div key={user.id} className="bg-white p-6 rounded-3xl shadow-sm border border-orange-100 flex flex-col relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1 bg-orange-400"></div>
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-xl font-black text-gray-900 leading-tight">{user.name || 'Без имени'}</h3>
                        <p className="text-sm font-bold text-gray-400 mt-1">@{user.login}</p>
                      </div>
                      <span className="text-[10px] font-bold uppercase px-2 py-1 rounded-md bg-orange-100 text-orange-700">
                        Ожидает
                      </span>
                    </div>
                    ...
                    <div className="mb-4 space-y-1">
                      {user.email && <div className="text-sm text-gray-500 font-bold truncate">📧 {user.email}</div>}
                      {user.role && <div className="text-sm text-gray-500 font-bold">👤 Роль: {user.role}</div>}
                    </div>

                    <div className="mt-auto flex flex-col gap-2 pt-4 border-t border-gray-50">
                      <button 
                        onClick={() => copyInviteLink(user.inviteToken)} 
                        className="w-full bg-[#F25C05] text-white py-2.5 rounded-xl font-bold text-xs hover:bg-orange-600 transition-colors shadow-md shadow-orange-500/20 flex justify-center items-center gap-2"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4">
                      ...
                      <button 
                        ...
                        className="w-full bg-red-50 text-red-500 py-2.5 rounded-xl font-bold text-xs hover:bg-red-100 transition-colors"
                      >
