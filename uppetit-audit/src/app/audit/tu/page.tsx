...
export default function TuLocationsPage() {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center font-bold text-gray-400 dark:text-zinc-500 transition-colors">Загрузка...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 md:bg-transparent dark:bg-zinc-950 dark:md:bg-transparent flex flex-col relative pb-20 transition-colors duration-300">
      ...
      <header className="bg-white/90 dark:bg-zinc-900/80 backdrop-blur-md p-4 sm:p-6 shadow-sm sticky top-0 z-20 border-b border-gray-100 dark:border-zinc-800 transition-colors">
        ...
        {locations && locations.length > 0 && (
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 sm:pl-12 pr-4 py-3 sm:py-3.5 rounded-xl border border-gray-100 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-950/50 focus:bg-white dark:focus:bg-zinc-950 focus:border-[#F25C05] dark:focus:border-[#F25C05] outline-none text-sm font-bold text-gray-700 dark:text-zinc-300 placeholder-gray-400 dark:placeholder-zinc-500 transition-colors shadow-inner"
            />
            {searchQuery && (
              <button 
                onClick={() => setSearchQuery('')}
                className="absolute inset-y-0 right-0 pr-3 sm:pr-4 flex items-center text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-zinc-300 font-bold transition-colors"
              >
                ✕
              </button>
            )}
      ...
      <main className="flex-1 p-4 space-y-4 pb-10 max-w-3xl mx-auto w-full relative z-10">
        {!locations || locations.length === 0 ? (
          <div className="text-center py-12 bg-white dark:bg-zinc-900 rounded-3xl border border-gray-100 dark:border-zinc-800 border-dashed transition-colors">
            ...
            <p className="text-gray-500 dark:text-zinc-400 font-bold text-sm transition-colors">У вас пока нет привязанных точек.</p>
          </div>
        ) : filteredLocations.length === 0 ? (
          <div className="text-center py-12 bg-white dark:bg-zinc-900 rounded-3xl border border-gray-100 dark:border-zinc-800 border-dashed transition-colors">
            <div className="text-4xl mb-3">🔍</div>
            <p className="text-gray-500 dark:text-zinc-400 font-bold text-sm transition-colors">По вашему запросу точки не найдены.</p>
          </div>
        ) : (
