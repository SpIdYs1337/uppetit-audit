...
export default function AuditHistoryPage() {
  ...
  // Сбор уникальных данных для фильтров
    ...
    return (
      <div key={filterKey} className={`relative ${isOpen ? 'z-50' : 'z-10'}`}>
        ...
        <div 
          ...
          className={`w-full p-3 rounded-xl bg-gray-50 border outline-none font-bold text-sm cursor-pointer flex justify-between items-center transition-colors ${isOpen ? 'border-[#F25C05] bg-white text-gray-900' : 'border-gray-100 text-gray-700 hover:bg-gray-100/50'}`}
        >
        ...
        {isOpen && (
          <>
            ...
            <div className="absolute z-50 top-full left-0 mt-2 w-full min-w-[240px] bg-white border border-gray-100 rounded-xl shadow-xl max-h-[300px] overflow-y-auto p-1.5 custom-scrollbar">
              ...
              {options.length === 0 && (
                <div className="p-3 text-center text-sm font-bold text-gray-400">Нет данных</div>
              )}
            </div>
          </>
        )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col relative pb-20">
      ...
      {/* ШАПКА */}
      <header className="bg-white p-4 sm:p-6 shadow-sm sticky top-0 z-30 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] sm:text-xs text-gray-400 font-bold mt-0.5 uppercase tracking-wider truncate">Аудиты на вашей территории</p>
          </div>
        </div>
        ...
        {filteredAudits.length > 0 && (
          <button 
            ...
            className="text-green-700 bg-green-50 hover:bg-green-100 border border-green-200 px-3 py-2 sm:px-4 sm:py-2 rounded-xl font-bold text-xs sm:text-sm transition-colors flex items-center gap-2 disabled:opacity-50"
          >
      ...
      <main className="flex-1 p-4 max-w-4xl mx-auto w-full flex flex-col gap-6 relative z-10">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-3xl shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            ...
            {isAnyFilterActive && (
              <button onClick={resetFilters} className="text-xs font-bold text-[#F25C05] hover:text-orange-600 bg-orange-50 px-3 py-1 rounded-lg transition-colors">
                ✕ Сбросить всё
              </button>
            )}
          </div>
        ...
        <div className="space-y-4">
          {paginatedAudits.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-3xl border border-gray-100 border-dashed">
              ...
              <p className="text-gray-500 font-bold">По вашему запросу аудитов не найдено.</p>
            </div>
          ) : (
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="text-sm font-bold text-gray-500">
              Стр. <span className="text-gray-900">{currentPage}</span> из {totalPages}
            </div>
            <button 
